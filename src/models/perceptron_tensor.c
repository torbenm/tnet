#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <math.h>

#include "core.h"
#include "models.h"
#include "funcs.h"

#define LEARNING_RATE 0.1

tensor *perceptron_tensor_forward(struct perceptron_tensor *p, tensor *vals)
{
    // tensor *r_mul = t_elem_mul(t_copy(p->weights), vals);

    // vec sum = vec_from_single(p->bias + vec_collapse_sum(r, p->numWeights));
    // free(r);
    // vec res = p->activationFn(sum, 1, FUNCS_NORMAL);
    // free(sum);
    // return res;
    return NULL;
}

int perceptron_tensor_train(struct perceptron_tensor *p, int maxIter, int numTVals, mat tvals, vec truth, param_t learningRate)
{
    // param_t errSum = 1.0;
    // int iter = 0;
    // while (errSum > 0.0 && iter < maxIter)
    // {
    //     errSum = 0.0;
    //     for (int i = 0; i < numTVals; i++)
    //     {
    //         vec res = perceptron_forward(p, tvals[i]);
    //         param_t error = truth[i] - res[0];
    //         errSum += fabs(error);
    //         vec deltaW = vec_mul_const(tvals[i], learningRate * error, p->numWeights);
    //         vec newWeights = vec_elem_add(p->weights, deltaW, p->numWeights);
    //         free(p->weights);
    //         free(deltaW);
    //         p->weights = newWeights;
    //         p->bias = p->bias + learningRate * error;
    //     }
    //     printf("Iter %i: error=%.3f\n", iter, errSum);
    //     iter++;
    // }
    // return iter;
    return 0;
}

struct perceptron_tensor *perceptron_tensor_init(int numWeights)
{
    struct perceptron_tensor *p = calloc(1, sizeof(struct perceptron_tensor));
    const int weights_shape[] = {numWeights};
    p->weights = t_alloc(1, weights_shape);
    t_init_rand(p->weights);

    const int bias_shape[] = {1};
    p->bias = t_alloc(1, bias_shape);
    t_init_rand(p->bias);

    p->activationFn = av_heaviside;
    return p;
}

void perceptron_tensor_free(struct perceptron_tensor *p)
{
    // free(p->weights);
    // free(p);
}
